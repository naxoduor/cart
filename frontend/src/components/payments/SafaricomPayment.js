...
function SafaricomPayment(props) {
    ...
    const handleSubmit = ((e) => {
        e.preventDefault();
        let amount = "";
        let phoneNumber = props.customer.item.mob_phone
        let msisdn=phoneNumber.replace(/^[0]/, '254');   // 2345
        let billRefNumber = props.trxNumber.transactionNumber
        props.cartAmount.items.map(amt => {
            if (amt.total_amount) {
        ...
        props.c2bSimulate(msisdn, amount, billRefNumber, null, null)
    })
        let phoneNumber = this.props.customer.item.mob_phone
        let msisdn=phoneNumber.replace(/^\+[0-9]/, '254');
        let total= 0;
        let deliveryTotal=0;
        props.orderDetails.items.map(product =>{
